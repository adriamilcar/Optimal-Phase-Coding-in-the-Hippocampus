...
from encoder_class.processing_functions_experiments import min_phi_distribution
from encoder_class.theoretical_functions_experiments import trunc_Gauss_Sample, phi_of_I_lin
...
def run_simulation(network_model, report_style = 'text', mode='experimental'):
    ...
    if mode=='theoretical':
        ...
        oscillation_T = network_model.oscillation_params["oscillation_T"]
        for ensemble in range(network_model.M):
            ...
            value, inter, sl = phi_of_I_lin(Vthres=network_model.model_params["v_thres"],
                                            ...
                                            I_0=ie, I=ie)
            phi_mean = value
            i_sigma = noise_frac*v_th/R_m
            exp_correction = np.sqrt((1 - np.exp(-2*oscillation_T/tau_m))/2)/((1 - np.exp(-oscillation_T/tau_m)))
            pre_sigma = exp_correction*i_sigma
            phi_std = np.abs(sl*pre_sigma)
            phis.append(trunc_Gauss_Sample(network_model.M, network_model.N, mu=phi_mean, sigma=phi_std, a=-pi,
                                    b=pi, num_oscillations=network_model.simulation_params["num_oscillations"]))
        phis = np.array(phis)
        return phis
