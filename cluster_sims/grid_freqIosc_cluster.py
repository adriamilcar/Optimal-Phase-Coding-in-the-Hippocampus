...
def grid_freqIosc_exp(idx):
    ...
    Ioscs = np.linspace(Iosc_lims[0], Iosc_lims[1], 100)
    fs = np.linspace(f_lims[0], f_lims[1], 100)

    experiments = list(itertools.product(Ioscs, fs))
    num_experiments = len(list(experiments))

    Iosc, f = experiments[idx]  #access

    oscillation_params["Iosc"] = Iosc
    ...
    filename = 'Data_Iosc/' + 'experimental_' + '{0:03d}'.format(int(idx/100)) + '_' + '{0:03d}'.format(int(idx%100)) + ".pickle"
