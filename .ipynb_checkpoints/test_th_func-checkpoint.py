...
from brian2 import *
from encoder_class.phase_encoder import PhaseEncoder
from encoder_class.theoretical_functions import *
...
def reset_encoder(f):
    ...
    return PhaseEncoder(num_ensembles=M, ensemble_size=N, model_params=model_params,
...
class TestThFunc(unittest.TestCase):
    ...
    def test_phi_of_I(self):
        encoder = reset_encoder(5*Hz)
        output = phi_of_I(encoder, 110*pA)
        result = 2.08051
        self.assertAlmostEqual(output, result, 3)
        
        encoder = reset_encoder(30*Hz)
        output = phi_of_I(encoder, 140*pA)
        result = 3.00794
        self.assertAlmostEqual(output, result, 3)       
    ...
    def test_phi_of_I_prime(self):
        encoder = reset_encoder(5*Hz)
        output = phi_of_I_prime(encoder, 110*pA)*amp*1e-10
        result = -3.16031
        self.assertAlmostEqual(output, result, 3)
        
        encoder = reset_encoder(30*Hz)
        output = phi_of_I_prime(encoder, 140*pA)*amp*1e-10
        result = -11.6236
        self.assertAlmostEqual(output, result, 3)
